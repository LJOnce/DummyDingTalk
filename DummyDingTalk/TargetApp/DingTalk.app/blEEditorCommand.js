...
BLEditor.Editor.prototype._formatState = null;
BLEditor.Editor.prototype.INDENT_MARGIN_WIDTH = 40;
BLEditor.Editor.prototype.INDENT_EDGE_MARGIN_WIDTH = 40;
BLEditor.Editor.prototype.INDENT_NOT_AT_MARGIN = 0;
BLEditor.Editor.prototype.INDENT_AT_LEFT_MARGIN = -1;
BLEditor.Editor.prototype.INDENT_AT_RIGHT_MARGIN = 1;

BLEditor.Editor.prototype.COMMANDS = {
	"bold": null,
	"italic": null,
	"underline": null,
	"strikeThrough": null,
...
BLEditor.Editor.prototype.NOARG_COMMANDS = {
	"bold": null,
	"italic": null,
	"underline": null,
	"strikeThrough": null,
...
BLEditor.Editor.prototype.FONT_COLOR_CHANGING_COMMANDS = {
	"createLink": null,
	"unlink": null,
	"backColor": null
};

BLEditor.Editor.prototype.FONT_CHANGING_COMMANDS = {
	"formatBlock": null,
	"createLink": null,
	"unlink": null
};

BLEditor.Editor.prototype.FORMAT_BLOCK_STYLES = {
	"BLOCKQUOTE": "margin: 1em; font-style: italic;",
	"P": "margin: 0px; padding: 0px;"
};

BLEditor.Editor.prototype.FONT_FORMAT_COMMANDS = {
	"fontName": null,
	"fontSize": null,
	"foreColor": null,
	"backColor": null
...
BLEditor.Editor.prototype.FORMAT_COMMANDS = {
	"bold": null,
	"italic": null,
	"strikeThrough": null,
	"underline": null,
...
BLEditor.Editor.prototype.INDENT_NODE_ATTRS = {
	"BLOCKQUOTE": {
		"class": "webkit-indent-blockquote"
	}
};

BLEditor.Editor.prototype.INDENT_NODE_STYLES = {
	"BLOCKQUOTE": "margin: 0 0 0 40px; border: none; padding: 0px;"
};

BLEditor.Editor.prototype.queryCommandValue = function(commandName) {
...
BLEditor.Editor.prototype.queryCommandEnabled = function(commandName) {
...
BLEditor.Editor.prototype.executeCommands = function(commands) {
	void(0);
	if (!commands) {
		return;
	}
...
BLEditor.Editor.prototype._prepareToExecuteCommands = function() {
	void(0);
	var editingStarted = this._hasEditingStarted;
	if (!editingStarted) {
		this.startEdit();
...
BLEditor.Editor.prototype._finalizeCommandExecution = function() {
	void(0);
};

BLEditor.Editor.prototype.isCommandChangingFont = function(command) {
...
BLEditor.Editor.prototype.isCommandChangingFontColor = function(command) {
...
BLEditor.Editor.prototype.getFontFormatState = function() {
	var state = {};
...
BLEditor.Editor.prototype.executeCommand = function(command, value) {
	void(0);
	var editingStarted = this._hasEditingStarted;
	this._prepareToExecuteCommands();
	var ret = false;
...
BLEditor.Editor.prototype._executeCommand = function(command, value) {
	void(0);
	if (this.COMMANDS[command] === undefined) {
		void(0);
		return false;
...
BLEditor.Editor.prototype.setFontFormatState = function(state, restrictCommandsObject) {
	if (!state) {
		return;
	}
	for (var command in this.FONT_FORMAT_COMMANDS) {
...
BLEditor.Editor.prototype.insertOrderedList = function(value) {
	void(0);
	return this.insertList("ol", value);
};

BLEditor.Editor.prototype._getListElementFromNode = function(node) {
	void(0);
...
BLEditor.Editor.prototype.insertUnorderedList = function(value) {
	void(0);
	return this.insertList("ul", value);
};

BLEditor.Editor.prototype._getSelectedListElement = function() {
	void(0);
	var sel = window.getSelection();
	if (sel.anchorNode) {
...
BLEditor.Editor.prototype._isSelectionSpanningEntireList = function() {
	void(0);
...
BLEditor.Editor.prototype.switchListType = function(listElement, tagName) {
	void(0);
	var sel = window.getSelection();
...
BLEditor.Editor.prototype.findBoundaryRanges = function(root, boundary, filter, max) {
	var ranges = [];
...
BLEditor.Editor.prototype.insertList = function(tagName, value) {
	void(0);
...
BLEditor.Editor.prototype.indent = function() {
	void(0);
	var ret = false;
	this.igoresChange(true);
	try {
...
BLEditor.Editor.prototype.isSelectionInsideNestedList = function() {
	var sel = window.getSelection();
...
BLEditor.Editor.prototype.outdent = function() {
	void(0);
	this.igoresChange(true);
	var ret = false;
	try {
...
BLEditor.Editor.prototype._onIndentationChange = function(root) {
	void(0);
...
BLEditor.Editor.prototype.getFormatState = function(force) {
	void(0);
...
BLEditor.Editor.prototype._setFormatState = function(state) {
	void(0);
...
BLEditor.Editor.prototype._resetFormatState = function _resetFormatState() {
	void(0);
...
BLEditor.Editor.prototype.formatBlock = function(value, preserveStyles) {
	void(0);
...
BLEditor.Editor.prototype._saveFormatState = function(force) {
	void(0);
	this._savedFormatState = this.getFormatState(force);
};

BLEditor.Editor.prototype._restoreSavedFormatState = function() {
	void(0);
	this._setFormatState(this._savedFormatState);
	this._savedFormatState = null;
};

BLEditor.Editor.prototype.insertCheckbox = function insertCheckbox(on) {
	void(0);
...
BLEditor.Editor.prototype.hasTextSelection = function() {
	void(0);
	var sel = window.getSelection();
	return (sel && !sel.isCollapsed);
};

BLEditor.Editor.prototype.restoreSelectionIgnoringDOMChanges = function() {
	void(0);
	this.igoresChange(true);
	try {
		this.restoreSelection();
...
BLEditor.Editor.prototype.escapeHTML = function(str) {
	void(0);
	return str.replace(/</, "&lt;").replace(/>/, "&gt;");
};

BLEditor.Editor.prototype.applyHyperlink = function(href, title) {
	void(0);
	var editingStarted = this._hasEditingStarted;
	if (!editingStarted) {
		this.startEdit();
...
BLEditor.Editor.prototype.resetHtmlToText = function() {
	this.igoresChange(true);
	this.rootElement.innerHTML = this.rootElement.textContent;
	this.setSelectionAtEnd();
	this.scrollToCaret(0 , BLEditor.Editor.scrollSourceEnum.resetHtml);
	this.igoresChange(false);
}

BLEditor.Editor.prototype.normalizeAdjacentTextNodes = function(textNode) {
	void(0);
	var sel = window.getSelection();
...
BLEditor.Editor.prototype.normalizeTextForElement = function(element, extraStyleAttrs) {
	void(0);
	if (element) {
		var styleAttr = null;
		if (extraStyleAttrs) {
...
////////////////////begin qureyCommand ////////////////////////////////////////
BLEditor.Editor.prototype.getCurrentSelectionContainer = function() {
	void(0);
	var sel = window.getSelection();
	if (sel.rangeCount > 0) {
...
BLEditor.Editor.prototype.queryFormatBlockEnabled = function() {
	var container = this.getCurrentSelectionContainer();
	return (container) ? true: false;
};

BLEditor.Editor.prototype.queryFontNameValue = function() {
	return this._queryFontAttribute("fontName");
};

BLEditor.Editor.prototype.queryFontSizeValue = function() {
	return this._queryFontAttribute("fontSize");
};

BLEditor.Editor.prototype.queryForeColorValue = function() {
	return this._queryFontAttribute("foreColor");
};

BLEditor.Editor.prototype.queryBackColorValue = function() {
	return this._queryFontAttribute("backColor");
};

BLEditor.Editor.prototype._queryFontAttribute = function(attribute) {
	var sel = window.getSelection();
...
BLEditor.Editor.prototype.getCurrentSelectionParentElementByTagName = function(tagName) {
	void(0);
	var sel = window.getSelection();
...
BLEditor.Editor.prototype.queryUnlinkValue = function() {
	var value = null;
...
BLEditor.Editor.prototype._hasSelectionWithLists = function(tagName) {
	void(0);
	var sel = window.getSelection();
	if (sel.isCollapsed) {
		return false;
...
BLEditor.Editor.prototype.queryInsertListEnabled = function(commandName) {
	var tagName;
...
BLEditor.Editor.prototype.queryInsertListValue = function(commandName) {
	var tagName;
	if (commandName == "insertUnorderedList") {
		tagName = "ul";
	}
...
BLEditor.Editor.prototype.queryInsertOrderedListEnabled = function() {
	return this.queryInsertListEnabled("insertOrderedList");
};

BLEditor.Editor.prototype.queryInsertOrderedListValue = function() {
	return this.queryInsertListValue("insertOrderedList");
};

BLEditor.Editor.prototype.queryInsertUnorderedListEnabled = function() {
	return this.queryInsertListEnabled("insertUnorderedList");
};

BLEditor.Editor.prototype.queryInsertUnorderedListValue = function() {
	return this.queryInsertListValue("insertUnorderedList");
};

BLEditor.Editor.prototype.getParentElement = function(nodeName, startNode) {
	void(0);
...
BLEditor.Editor.prototype.queryIndentationEnabled = function(commandName) {
	var sel = window.getSelection();
	var baseNode = sel.baseNode;
...
BLEditor.Editor.prototype._isIndentedNodeAtMargin = function(node) {
	void(0);
	if (node.nodeType != Node.ELEMENT_NODE) {
		node = node.parentElement;
	}
...
BLEditor.Editor.prototype.queryIndentEnabled = function() {
	return this.queryIndentationEnabled("indent");
};

BLEditor.Editor.prototype.queryOutdentEnabled = function() {
