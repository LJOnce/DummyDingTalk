...
void main()
{
    ...
    rgb *= d;
    outputColor.rgb            = rgb;
    lowp float redCurveValue   = texture2D(toneCurveTexture, vec2(outputColor.r, 0.0)).r+0.1;
    lowp float greenCurveValue = texture2D(toneCurveTexture, vec2(outputColor.g, 0.0)).g+0.1;
    lowp float blueCurveValue  = texture2D(toneCurveTexture, vec2(outputColor.b, 0.0)).b+0.1;
